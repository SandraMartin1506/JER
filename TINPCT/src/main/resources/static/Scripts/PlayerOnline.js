class PlayerOnline extends Phaser.GameObjects.Group
{
    constructor(initialX, initialY, scene, spriteSheet, hat, top, bottom, deadSprite)
    {
        super({key: "PlayerOnline"});
        //Skin:
    ...
    ManageInput(scene) //Añade al evento keydown la función de detección de direcciones del jugador
    {
        scene.input.keyboard.on('keydown', function (event)
        {
            ...
            var msg = {type: "MoveP1", key: event.key, inputType: "move"}
	...
	MovePlayer(key)
	{
		if(key === 'w' && this.currentInput !== 'w') //Con currentInput se hace que sólo se registre una vez la pulsación
            {
            ...
            else if (key === 's' && this.currentInput !== 's')
            {
            ...
            else if (key === 'a' && this.currentInput !== 'a')
            {
            ...
            else if (key === 'd' && this.currentInput !== 'd')
            {
    ...
    StopMovement(scene) //Añade al eveneto keyup la detención de movimiento del jugador
    {
        ...
        {
            this.StopPlayer(event.key);
            var msg = {type: "MoveP1", key: event.key, inputType: "stop"};
            window.socket.send(JSON.stringify(msg));
        }.bind(this))
    }
	...
	StopPlayer(key)
	{
		if(this.currentInput === key) //Sólo se para si la tecla que ha dejado de pulsarse es la que hacia que se moviese
        	{
